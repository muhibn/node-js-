...
export class JobTitleComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['select','jobTitleId', 'jobTitleName', 'jobDescription', 'createdBy', 'createdOn', 'updatedBy', 'updatedOn', 'isActive', 'actions'];
 ...
 ///////For Check_Box////////////
 selection = new SelectionModel<jobTitleList>(true, []);
 ...
 checkboxLabel(row?: jobTitleList): string {
   if (!row) {
     return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
   }
   return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row ${row.jobTitleId+1}`;
}
