...
export class WorkTypeComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['select','workTypeId', 'workTypeName', 'createdBy', 'createdOn', 'updatedBy', 'updatedOn', 'isActive', 'actions'];
 ...
 ///////For Check_Box////////////
 selection = new SelectionModel<worktypeList>(true, []);
 ...
 checkboxLabel(row?: worktypeList): string {
   if (!row) {
     return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
   }
   return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row ${row.workTypeId+1}`;
}
