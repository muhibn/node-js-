...
export class RoleComponent implements AfterViewInit{
  ...
  displayedColumns: string[] = ['select','roleId', 'roleName', 'createdBy', 'createdOn', 'updatedBy', 'updatedOn', 'isActive', 'actions'];
...
///////For Check_Box////////////
selection = new SelectionModel<roleList>(true, []);
...
checkboxLabel(row?: roleList): string {
  if (!row) {
    return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
  }
  return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row ${row.roleId+1}`;
}
