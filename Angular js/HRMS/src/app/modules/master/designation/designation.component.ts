...
export class DesignationComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['select','desigId', 'departmentName','designationName', 'createdBy', 'createdOn', 'updatedBy', 'updatedOn', 'isActive', 'actions'];
...
registration: any;
 ...
 ///////For Check_Box////////////
 selection = new SelectionModel<designationList>(true, []);
 ...
 checkboxLabel(row?: designationList): string {
   if (!row) {
     return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
   }
   return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row ${row.desigId+1}`;
}
