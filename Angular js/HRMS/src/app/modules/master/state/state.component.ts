...
export class StateComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['select','stateId', 'country', 'stateName', 'createdBy', 'createdOn', 'updatedBy', 'updatedOn', 'isActive', 'actions'];
 ...
 ///////For Check_Box////////////
 selection = new SelectionModel<stateList>(true, []);
 ...
 checkboxLabel(row?: stateList): string {
   if (!row) {
     return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
   }
   return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row ${row.stateId+1}`;
}
