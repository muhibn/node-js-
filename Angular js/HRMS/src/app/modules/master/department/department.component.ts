...
export class DepartmentComponent implements AfterViewInit {
  ...
  displayedColumns: string[] = ['select','deptId', 'departmentName', 'createdBy', 'createdOn', 'updatedBy', 'updatedOn', 'isActive', 'actions'];
 ...
 checkboxLabel(row?: departmentList): string {
   if (!row) {
     return `${this.isAllSelected() ? 'deselect' : 'select'} all`;
   }
   return `${this.selection.isSelected(row) ? 'deselect' : 'select'} row ${row.deptId+1}`;
}
